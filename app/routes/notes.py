...
async def update_note_route(note_id: str, title: Annotated[str, Body()], content: Annotated[str, Body()], current_user: Annotated[str, Depends(get_current_user)]):
...
async def delete_note_route(note_id: str, current_user: Annotated[str, Depends(get_current_user)]):
    ...
    if note["collaborators"][0] != current_user:
        raise HTTPException(403, "Only the owner can delete this note")
    result = delete_note_by_id(note_id)
    if result.deleted_count == 1:
        return {"message": "Note deleted successfully"}
    raise HTTPException(500, "Note deletion failed")